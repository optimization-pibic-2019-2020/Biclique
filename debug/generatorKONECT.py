...
for (inputPath,outputPath) in folders:
	...
	for (x, y) in files:
		...
		partition1_count, partition2_count, edge_count = [int(z) for z in next(f).split(' ')]
		vertex_count = partition1_count + partition2_count
		edges = []
		...
		for edge_list in edges:
			s.write(str(edge_list[0]) + " " + str(edge_list[1] + partition1_count))
			s.write("\n")
		f.close()
		s.close()
